...
**Q：快捷指令一直在转圈，最后提示超时/失败？**
A：这 100% 是物理网络阻断。请按以下步骤排查：
1. **真实 IP 问题**：如果您电脑开了虚拟网卡（如某些代理软件的 TUN 模式），可能会导致二维码里的 IP 错误。请**暂时关闭电脑和手机的代理软件**，重新打开 PhoneBridge 生成二维码并扫码测试。
2. **防火墙问题**：请确保 Windows 防火墙允许 PhoneBridge 通过，或者在测试时暂时关闭“专用网络”和“公用网络”防火墙。
3. **局域网隔离**：请确保手机和电脑连接在同一个 Wi-Fi 或热点下，且路由器没有开启“AP 隔离”。如果您手机开了代理（如 Surge/Shadowrocket），必须在代理设置中开启**“绕过局域网 (Bypass LAN)”**。

---

### 5. 如何彻底卸载

由于 PhoneBridge 会向系统注册开机自启服务，直接删除 `.exe` 文件会导致注册表残留。
**正确卸载步骤**：
1. 在右下角系统托盘找到 PhoneBridge 图标。
2. 右键点击，**取消勾选“开机启动”**。
3. 点击“退出”彻底关停程序。
4. 此时您可以安全地删除 `phonebridge.exe` 和 `C:\Users\用户名\.phonebridge` 配置文件夹，不留任何痕迹。
